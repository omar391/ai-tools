...
fn build_tray_icon() -> Image<'static> {
    ...
    for y in 0..height {
        for x in 0..width {
            ...
            let arrow_head = (x >= 11 && x <= 16) && (y >= 2 && y <= 7) && (x as i32 - y as i32 >= 8);
...
fn run_bun_json(args: &[&str]) -> Result<serde_json::Value, String> {
    ...
    let package_entry = repo_root.join("packages").join("codex-rotate-app").join("index.ts");
...
fn rotate_next_and_switch() -> Result<WatchOnceEnvelope, String> {
    let value = run_bun_json(&["rotate-next-and-switch", "--port", &DEFAULT_PORT.to_string()])?;
    ...
    serde_json::from_value(wrapped).map_err(|error| format!("failed to decode rotation result: {error}"))
}
...
fn main() {
    tauri::Builder::default()
        .setup(|app| {
            ...
            let launch_item = MenuItem::with_id(app, "launch", "Open Wrapper Codex", true, None::<&str>)?;
                        ...
                        "launch" => {
                            ...
                            thread::spawn(move || {
                                let next = if let Err(error) = launch_codex() {
                                    let mut snapshot = status.inner.lock().expect("status mutex");
                                    snapshot.last_message = Some(format!("launch failed: {}", error));
                                    snapshot.clone()
                                } else {
