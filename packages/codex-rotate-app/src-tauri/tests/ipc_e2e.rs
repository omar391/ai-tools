...
fn built_cli_binary() -> Result<PathBuf, Box<dyn std::error::Error>> {
    ...
    let status = Command::new("cargo")
        ...
        .status()?;
    if !status.success() {
        return Err(format!("Failed to build codex-rotate-v2 via {}.", manifest_path.display()).into());
    }

    let path = workspace_root.join("target/debug/codex-rotate-v2");
    BUILT_CLI
        .set(path.clone())
...
#[test]
fn tray_shell_launches_real_v2_daemon_binary() -> Result<(), Box<dyn std::error::Error>> {
    let _guard = lock_unpoisoned(env_mutex());
    let sandbox = unique_temp_dir("codex-rotate-v2-tray-launch");
    ...
    let wrapper = sandbox.join("codex-rotate-v2-wrapper.sh");
...
fn tray_shell_auto_starts_and_streams_real_daemon_snapshots() -> Result<(), Box<dyn std::error::Error>> {
    let _guard = lock_unpoisoned(env_mutex());
    let sandbox = unique_temp_dir("codex-rotate-v2-tray-ipc");
    let rotate_home = sandbox.join("rotate-home");
    let codex_home = sandbox.join("codex-home");
    fs::create_dir_all(&rotate_home)?;
    fs::create_dir_all(&codex_home)?;
